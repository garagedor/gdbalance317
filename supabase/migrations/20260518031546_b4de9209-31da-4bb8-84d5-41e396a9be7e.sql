...
BEGIN
  ...
  -- LM parts are subtracted from profit just like company_parts/tech_parts.
...
-- Recompute existing jobs so lm_parts is reflected
UPDATE public.weekly_report_jobs SET updated_at = now() WHERE COALESCE(lm_parts, 0) > 0;