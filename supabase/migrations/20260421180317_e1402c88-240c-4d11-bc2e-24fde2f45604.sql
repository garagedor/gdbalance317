...
BEGIN
  ...
  -- Payment fee: card 5%, finance 10%, company check 10%; cash & company cash 0%.
  -- Tips are NOT included in payment_fee — they are netted directly in tips total.
  _payment_fee := ROUND(
      NEW.paid_card          * 0.05
    + NEW.paid_finance       * 0.10
    + NEW.paid_company_check * 0.10, 2);

  -- Net tips: card 95%, finance 90%, check 90%, company cash 100%.
  _tips := ROUND(
      NEW.tips_card          * 0.95
  ...
  -- Mirror legacy fields (kept for older table views/reports).
  ...
  NEW.card_fee_base      := NEW.paid_card + NEW.paid_finance + NEW.paid_company_check;
