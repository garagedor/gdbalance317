-- Make Area Manager self-job commission configurable per-user.
-- Previously hardcoded to 0.40. Now read from users.commission_rate;
-- if the AM's stored rate is the technician default (0.30) or NULL, fall back
-- to 0.40 for backwards compatibility. Admin can set any value 0..1.
...
BEGIN
  ...
  IF _user_role = 'area_manager' THEN
    -- Personal jobs performed by an Area Manager use their configured
    -- self-job commission rate. Fall back to 0.40 when not set or still
    -- at the technician default of 0.30.
    NEW.commission_rate := COALESCE(NULLIF(_user_rate, 0.30), 0.40);
...
-- 4) Backfill: AMs whose commission_rate is still 0.30 (technician default)
--    should be set to 0.40 so existing managers keep current behavior.
UPDATE public.users
