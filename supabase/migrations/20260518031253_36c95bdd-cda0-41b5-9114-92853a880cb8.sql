...
BEGIN
  ...
  -- Job total = sum of payment buckets only (tips are NOT part of job_total).
  -- LM cash and LM check are counted as payment methods (no processing fee).
  _job_total := ROUND(
  ...
  -- Payment fee: card 5%, finance 10%, company check 10%; cash, company cash,
  -- LM cash, and LM check are 0%. Tips are netted separately.
  _payment_fee := ROUND(
      NEW.paid_card          * 0.05
  ...
  -- Mirror legacy columns for backward compat
