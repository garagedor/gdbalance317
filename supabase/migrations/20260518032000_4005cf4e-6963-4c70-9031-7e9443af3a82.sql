-- 1) Add manager_profit_percent to areas (default 40%)
ALTER TABLE public.areas
  ADD COLUMN IF NOT EXISTS manager_profit_percent numeric NOT NULL DEFAULT 40;

ALTER TABLE public.areas
  DROP CONSTRAINT IF EXISTS areas_manager_profit_percent_range;
ALTER TABLE public.areas
  ADD CONSTRAINT areas_manager_profit_percent_range
  CHECK (manager_profit_percent >= 0 AND manager_profit_percent <= 100);

-- 2) Update per-job trigger to charge 10% fee on lm_check (lm_cash stays fee-free)
...
BEGIN
  ...
  IF COALESCE(NEW.lm_parts, 0) < 0 THEN RAISE EXCEPTION 'lm_parts must be >= 0'; END IF;
  IF COALESCE(NEW.lm_cash,  0) < 0 THEN RAISE EXCEPTION 'lm_cash must be >= 0'; END IF;
  IF COALESCE(NEW.lm_check, 0) < 0 THEN RAISE EXCEPTION 'lm_check must be >= 0'; END IF;

  -- job_total includes lm_cash + lm_check at full value
  _job_total := ROUND(
      NEW.tech_paid_cash + NEW.paid_card + NEW.paid_company_cash
    + NEW.paid_company_check + NEW.paid_finance
    + COALESCE(NEW.lm_cash, 0) + COALESCE(NEW.lm_check, 0), 2);

  -- Fees: card 5%, finance 10%, company check 10%, lm_check 10% (HARD RULE).
  -- Cash, company cash, lm_cash are 0%.
  _payment_fee := ROUND(
      NEW.paid_card          * 0.05
    + NEW.paid_finance       * 0.10
    + NEW.paid_company_check * 0.10
    + COALESCE(NEW.lm_check, 0) * 0.10, 2);

  _tips := ROUND(
      NEW.tips_card          * 0.95
    + NEW.tips_finance       * 0.90
    + NEW.tips_check         * 0.90
    + NEW.tips_company_cash, 2);

  -- lm_parts is a parts cost like tech_parts/company_parts.
  _total_profit       := ROUND(_job_total - NEW.tech_parts - NEW.company_parts - COALESCE(NEW.lm_parts, 0) - _payment_fee, 2);
  ...
  NEW.tech_cash         := NEW.tech_paid_cash;
  NEW.company_cash      := NEW.paid_company_cash;
  ...
  NEW.tip_amount        := NEW.tips_card + NEW.tips_finance + NEW.tips_company_cash + NEW.tips_check;
  NEW.total_job         := _job_total;
  NEW.card_fee_amount   := ROUND(NEW.paid_card * 0.05, 2);
  ...
  NEW.company_70        := ROUND(_total_profit - _tech_payout, 2);
  NEW.tech_payout       := _tech_payout;
  NEW.company_total     := ROUND((_total_profit - _tech_payout) + NEW.company_parts, 2);
...
-- Recompute existing jobs that have lm_check so the new fee is applied
