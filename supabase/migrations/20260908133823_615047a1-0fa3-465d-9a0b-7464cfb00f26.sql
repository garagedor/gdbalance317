...
BEGIN
  ...
  IF NOT (
    ...
    OR (OLD.status = 'Approved'     AND NEW.status IN ('Returned','Under Review'))
  ) THEN
    RAISE EXCEPTION 'Invalid status transition: % -> %', OLD.status, NEW.status;
  END IF;

  IF OLD.status = 'Approved' AND NOT _is_mgmt THEN
    RAISE EXCEPTION 'Only management can reopen an approved report';
  END IF;
